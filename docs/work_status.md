...
|---:|---|---|---|---|---|---|---|
| 0-1 | プロジェクト作業フォルダ作成（docker/wp-content/docs） | 期待の構成で作成済み | 自分 | ⬜ |  |  |  |
| 0-2 | docker-compose.yml 作成 | WP+DBが起動する | 自分 | ⬜ |  |  |  |
| 0-3 | wp-content をホストにマウント | VS Code編集が即反映 | 自分 | ⬜ |  |  |  |
| 0-4 | WP初期設定（管理者作成） | /wp-admin ログイン可 | 自分 | ⬜ |  |  |  |
| 0-5 | プラグイン雛形作成（diarycoach.php） | 管理画面で有効化できる | 自分 | ⬜ |  |  |  |
| 0-6 | Git初期化・.gitignore 設定 | .env/DB等が除外される | 自分 | ⬜ |  |  |  |
